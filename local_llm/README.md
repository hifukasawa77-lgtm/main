...
リポジトリに **学習済みのsmallチャットモデル**（10.2Mパラメータ）と対応トークナイザを同梱しています。
日本語Wikipedia＋青空文庫 約2,500万トークンで事前学習（検証perplexity 79）→ dolly-15k-ja でSFT済み。
セットアップ後すぐに試せます:
...
※ smallサイズなので応答は「日本語の体裁が整ったヨチヨチ歩き」レベルです。
   文法は通りますが内容の正確さは期待しないでください。本格的な賢さは
   Step 1 以降の base 学習＋長時間学習で目指します。
