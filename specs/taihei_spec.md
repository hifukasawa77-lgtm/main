...
2. 全国マップ画面（`MapScene`）で年単位のターン制ループ:
...
**Must**
- 全国マップ（国ノード＋隣接、`genpei.html`/`sengoku.html`の province データを流用）とターン制進行（年単位、1シナリオ最大61ターン目安）
- 3陣営（北朝・足利方／南朝方／中立・地方勢力）・史実準拠の初期配置（4.2節の全30武将データ）
...
- 「観応の擾乱」単独短編シナリオ（1350年開始、~15ターン、内訌に絞った上級者向けリプレイ用）
...
- **FactionSelect**: 3陣営（北朝・足利方／南朝方／中立・地方勢力から代表家を選択）のカード表示、家紋・当主・思想傾向を提示
