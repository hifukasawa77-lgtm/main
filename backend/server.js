...
// ─── Claude AI エージェント API ──────────────────────────────

const CLAUDE_API_URL = 'https://api.anthropic.com/v1/messages';
const CLAUDE_MODEL = 'claude-sonnet-4-6';

async function callClaude(apiKey, system, userMessage, maxTokens = 3000) {
...
app.post('/api/agent/debug', async (req, res) => {
  ...
  const system = `あなたはHTML/JavaScriptゲームの専門コードレビュアーです。
提出されたゲームコードを以下の観点で分析し、必ずJSON形式のみで返してください（説明文等は不要）:
{
  "summary": "全体的な評価（1-2文）",
  "score": 0から100の整数,
  "bugs": [{"title": "問題名", "severity": "pass|warn|fail", "description": "説明", "suggestion": "改善案"}],
  "performance": [{"title": "...", "severity": "pass|warn|fail", "description": "...", "suggestion": "..."}],
  "security": [{"title": "...", "severity": "pass|warn|fail", "description": "...", "suggestion": "..."}],
  "quality": [{"title": "...", "severity": "pass|warn|fail", "description": "...", "suggestion": "..."}]
}
severity: pass=問題なし, warn=注意, fail=要修正。各カテゴリ2〜4件程度。`;

  try {
    const data = await callClaude(apiKey, system, code);
    res.json(data);
  } catch (err) {
...
app.post('/api/agent/spec', async (req, res) => {
  const apiKey = req.headers['x-api-key'];
  if (!apiKey) return res.status(400).json({ error: 'APIキーが必要です' });
  const { idea } = req.body;
  if (!idea) return res.status(400).json({ error: 'ゲームアイデアが必要です' });

  const system = `あなたはHTML5ゲーム設計の専門家です。
ゲームアイデアから詳細な仕様書をマークダウン形式で生成してください。以下の構成で出力:
...
## 基本設計書
- 画面構成（Canvasサイズ・UI配置）
- ゲームの状態管理（state machine）
- 主要データ構造
- ゲームループ設計（requestAnimationFrame）
...
## Canvas API コードスケルトン
動く最小限の雛形コード。HTML/CSS/JavaScript完結、フレームワーク不使用。
コードは\`\`\`html...\`\`\`で囲むこと。`;

  try {
    const data = await callClaude(apiKey, system, idea, 4096);
    res.json(data);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
app.post('/api/agent/achievement', async (req, res) => {
  const apiKey = req.headers['x-api-key'];
  if (!apiKey) return res.status(400).json({ error: 'APIキーが必要です' });
  const { code } = req.body;
  if (!code) return res.status(400).json({ error: 'コードが必要です' });

  const system = `あなたはゲームデザイナーです。
提出されたHTML/JavaScriptゲームのコードを読み解き、
ゲームメカニクスに基づいたユニーク・ユーモラスな実績称号を20個生成してください。
必ずJSON形式のみで返してください（他のテキスト不要）:
{
  ...
  "achievements": [
    {
      "name": "称号名（日本語、ユーモラスに）",
      "description": "達成条件の説明（1文）",
      "rarity": "common|rare|epic|legendary",
      "condition_code": "// 達成判定のJSコメント（例: score >= 1000 かつ lives === 3）",
      "icon": "絵文字1つ"
    }
  ]
}
rarity: common=普通, rare=珍しい, epic=すごい, legendary=伝説。
称号名は遊び心とユーモアを持たせること。コードに登場する変数名・ロジックを参照して具体的に。`;
