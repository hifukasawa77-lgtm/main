...
## レシートOCR家計簿の必須チェック（receipt-ocr.html を触ったら必ず実行）

```bash
node scripts/verify-receipt-ocr.mjs         # 解析→画像補正→取り込み〜家計簿→XSS→保存領域なし（51項目・偽OCRで速い）
node scripts/verify-receipt-ocr.mjs --ocr   # 実物のTesseractで読む（npmから本体と日本語モデルを取得・数分）
```

- **「◯件の品目を抽出」が出た＝読めている、ではない**。2026-09-25 まで本ページは、Tesseract の
  日本語モデルが数字を**「①②③」（丸数字）**、¥ を **「\」** で返すせいで価格を1件も拾えず、
  例外も出さずに「0件の品目を抽出」と表示していた（合成レシート12枚で0品目）。
  解析は `normalizeLine()` を通してから行う。検査は抽出した**中身**まで見る
- **精度の判定は「品目の計＝レシートの合計」**。正解を知らなくても分かる強い手掛かりなので、
  合わなければ補正・読み方を変えて読み直し（最大3回）、合った読み取りを採る。合計は
  「合計」行・お預り−お釣り・小計＋外税の3つの手掛かりを突き合わせる（「合計」は「合。言」に化ける）
- **傾き検出の投影は、ビン幅を間引き幅に合わせる**。1pxビンだと0°/90°だけ格子点が重なって
  偽のピークになり、傾いた写真を「0°」と判定していた
- **紙の検出に大津の閾値を使わない**。影の掛かった紙が机側に落ちて切り捨てられる。
  外周（机）から「なだらかに続く・机の色の画素」をたどり、明るさが跳ぶ紙の縁で止める
- **机を白く塗るのは「傾き補正後の紙の外接矩形」の外だけ**。画素単位の紙マスクで塗ると、
  濃い影の所でマスクが紙へ食い込み**価格の列ごと無言で消える**（机が灰色だと色でも区別できない）。
  検査は灰色の机＋濃い影で「右端の列のインク比率」を見る（影あり/なしの比較だけだと、両方同じだけ
  消える壊れ方を見逃した——故障注入で実際にすり抜けた）
- 精度の実測（合成レシート12枚・`--ocr` 相当、2026-09-25）: 旧版 0品目 → スキャン96%・スマホ撮影88%
  （撮影は4枚とも合計一致）。**強いぼけ（文字高の1割近いぼけ＋粗いJPEG）は4%のまま**＝前処理では
  救えない。そのときは撮り直しの案内を出す
- **日本語モデルは `@tesseract.js-data/jpn@1.0.0/4.0.0_best_int`（2MB）**。float版の
  tessdata_best は tesseract.js-core 7 が `DotProductSSE` 欠落で**クラッシュ**する。
  4.0.0（16MB）は撮影画像で best_int より低い（68% 対 88〜92%。合成レシートでの実測）
...
- **noteプレミアム（月500円）は前提ではない**。無料会員なら毎週1回貼って公開する
  （1本3分ほど）だけで同じ運用ができる。プレミアムで得られるのは予約投稿（Web版限定）＝
  「公開日に手を動かさなくてよい」ことだけで、**販売手数料は会員種別で変わらない**（利用料10%）
