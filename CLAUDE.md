...
- **地図の切り取り位置は2箇所にある**。`bakumatsu.css` の `.strategic-map` の
  `background-position` と `game.js` の `MAP_FOCUS` は必ず同じ値にすること。片方だけ直すと
  拠点だけが無言で地図から浮く。検査#1がこの一致を機械検査する
- 座標は元画像 `assets/maps/strategic-japan.png`（1672×941）に対する百分率。
  表示領域（674×488）は `cover` で**左右が切り取られる**ため、可視範囲は約 x 8.5〜86.2% しかない。
  九州西岸の拠点（佐賀・鹿児島）が枠から見切れるので `MAP_FOCUS.x` を中央より左（0.38）へ寄せてある
