...
**実装フェーズ**: Phase 1（地図・拠点・ターン進行）と Phase 2（名分・御恩と奉公・朝敵認定・経済）が動作。
`scripts/verify-genpei-boot.mjs` 全18項目 PASS（http / file:// 両方）。

次は Phase 3（ヘックス合戦・騎射・名乗り・一騎討ち）。コマンドバーの「軍事」がここで開通する。
