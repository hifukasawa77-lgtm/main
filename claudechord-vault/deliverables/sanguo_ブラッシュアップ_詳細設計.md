...
```
const snap = await page.evaluate(() => window.__sanguoDebug ? window.__sanguoDebug.owners() : null);
```

> `state` は IIFE 内に閉じているため外から読めない。
> **検査用に最小限のフックを1つだけ公開する**:
> `window.__sanguoDebug = { owners: () => CITIES.map(c=>c.owner), ap: () => [state.ap, state.apMax] };`
> グローバルを増やす例外だが、**検査以外から参照しない読み取り専用**とし、
> `CLAUDE.md` の「グローバルを増やさない」規約への意図的な例外としてコメントを残す。

判定:

| 検査 | 式 |
|---|---|
| #5 最大勢力 | 50巡後の `owners` の最頻値の出現数 ≧ 8 |
| #6 churn | 各都市について所有者が変わった回数を数え、`(変化回数 / 50) * 20 ≦ 3` |
| #7 圧力 | プレイヤー勢力の都市数が0になった巡 ≦ 25 |
| #8 武将空白 | 占領ログが出た巡の直後、`CITIES.filter(c=>c.generals.length===0).length` が前巡より増えていない |
