...
| BattleScene（Phase 3 実装済） | 13×9のヘックス盤・隊ごとの士気バー・兵種色・選択隊の能力と間合いの説明・戦況ログ・命令（名乗り／待機／この合を終える）|
| BattleScene naval（Phase 4 実装済） | 一面の海（両端が岸）・**潮目インジケータ**（向き・順逆・あと何合で変わるか）・選択隊の潮欄・水軍の離反ログ |

> 拠点アイコンは **Canvas のみで描く**（画像アセットに依存しないので、後読みの失敗で白い箱にならない）。
> コマンドバーのうち Phase 1 で機能するのは「情報・機能・ターン終了」。他は押すと開通予定の Phase を告げる。
>
> ### 地図は写真をそのまま使い、拡大率に上限を掛ける（決定・2026-08-05）
>
> **三段すべてで `sengoku-japan-map-user-v1.webp` をそのまま拡大して使う。**
> 素材が 1672×941 しかないため、引き伸ばすほどぼやける。
> **写真の質感を優先し、代わりに拡大率へ上限を置く**という深澤の判断。
>
> 上限は定数 `ZOOM_CAP`（原寸の何倍まで引き伸ばすか）。各段の寄りはこの上限に合わせて
> 自動的に浅くなる。**数値を上げれば寄れるがぼやける**——調整はここだけを触ればよい。
>
> | 段 | `ZOOM_CAP` | 実効拡大率 | 視野（原画素の幅） |
> |---|---|---|---|
> | 全国図 | `nation: 2.0` | 上限 2.0倍 | 347px |
> | 国の拡大図 | `prov: 3.0` | 3.0倍（従来 6.2倍） | 243px |
> | 拠点の周辺図 | `local: 4.5` | 4.5倍（従来 8.8倍） | 104px |
>
> 上限を掛けたぶん一画面に入る拠点が増えるため、**地名は重なり回避つきで描く**
> （選択中 → 国府・館 → その他 の順に置き、入らない札は落とす）。
>
> ⚠ **海岸線を抽出してベクターで描き起こす案は一度実装し、差し戻した**（コミット `ee07253` と
> その revert）。鮮明にはなるが地図の見た目が変わってしまうため。**同じ手を再提案しないこと。**
> さらに鮮明さが要る場合の残る手は「より大きな地図素材を用意する」のみ。
...
**`paperRule` — 知行国主であって主人ではない（実装時に追加）**

平氏は開始時に全国の国衙を握るが、坂東・東山・北陸の武士団は**平氏の家人ではない**。
そこで勢力に `paperRule: true` と `heartland`（本貫の道／国）を持たせ、
**本貫の外にある武士団は拠点の持ち主が誰であれ中立から始まる**ものとした。

```js
taira: { paperRule: true, heartland: ['kinai','sanyodo','nankaido','saikaido'],
         heartlandProvinces: ['iga','ise','shima'] }
```

これが「**版図は最大なのに動員が伸びない**」という平氏の史実の弱点になり、
同時に勧誘（4.5）が働く中立の池をつくる。1180シナリオで中立16団・鎌倉18団。
...
**別ファイルは作らず拠点データから決定論的に起こす**（館・城柵・武家領の荘園が本領）。
拠点を足すだけで武士団も増え、データの二重管理が起きない。名の通った党だけ
`BAND_NAMES` で呼び名を与え、載っていない拠点は「〈拠点名〉衆」になる。計65団。
