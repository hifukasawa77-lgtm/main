...
> ### 地図の描き分け（拡大時のぼやけ対策）
>
> 地図素材は `sengoku-japan-map-user-v1.webp`（1672×941）**しかなく、より大きなものは存在しない**。
> 国の拡大図は横 0.06 相当（約100px）を 726px へ伸ばすため **7倍以上の拡大**になり、
> 写真を貼る限り必ずぼやける。よって次のように描き分ける。
>
> | 段 | 倍率 | 描き方 |
> |---|---|---|
> | 全国図（等倍付近） | 〜2.2倍 | **写真**。この範囲では写真の方が美しい |
> | 全国図（拡大時） | 2.2倍〜 | **ベクター**へ切り替え |
> | 国の拡大図 | 約7倍 | **ベクター** |
> | 拠点の周辺図 | 約15倍 | **ベクター** |
>
> ベクター地図の輪郭は `assets/genpei/coastline.json`。地図画像から
> `scripts/gen-genpei-coastline.mjs` が抽出する（海岸線＋明度の分位点で切った標高帯2段）。
> 線とベタ塗りは何倍に拡大しても鮮明なので、拡大するほど破綻する問題が消える。
>
> ⚠ **毎フレーム描き直すと 60fps を割る**（1万点超のパス構築。実測 3.4倍で 43fps）。
> カメラが動かない限り同じ絵になるので、**オフスクリーンに焼いて使い回す**（実測 61fps）。
