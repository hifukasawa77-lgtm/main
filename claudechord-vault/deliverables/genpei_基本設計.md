...
| `kyoten_ichi.csv` | **拠点データの正本**（約145拠点）。リポジトリ直下 | 新規 |
| `assets/genpei/provinces.json` | 令制国65。`assets/sengoku/provinces.json` を複製し `kokudaka` を12世紀の田数相当へ置換 | 新規（複製元は既存） |
...
| 画面 | UI要素 |
|---|---|
| MapScene 上部 | 年月（治承四年 八月）／名分ゲージ／兵糧／金／朝敵バッジ／ターン終了 |
| MapScene 右 | 選択拠点パネル（種別・規模・防御・駐留武士団・占領/勧誘/安堵の行動）|
| MapScene 左 | 勢力一覧（名分順）・武士団一覧（奉公度順・危険な団を強調）|
| MapScene 下 | 年代記ログ（縦書き・巻物風）|
| 名分パネル | 内訳の分解表示（令旨/院宣/宣旨/官位/神器/評判/朝敵）※ Could C-03 |
| 恩賞パネル | 恩賞債務の総額・団別内訳・本領安堵/新恩給与の実行 |
| BattleScene | ヘックス盤・部隊士気バー・兵種アイコン・名乗りボタン・（naval）潮向きインジケータ |
...
拠点マーカーは**ズーム倍率で段階表示**（低倍率＝国衙のみ、高倍率＝荘園・館・湊も）。
...
| type | `scale` の意味 | 占領効果 | 無血開城 |
|---|---|---|---|
| `kokuga` 国衙 | 田数（町）＝税収・動員上限 | 国の徴税権と動員枠を得る | **可** |
| `shoen` 荘園 | 田数（町）＝兵糧収穫 | 兵糧収入。ただし `holder` が `jisha`/`sekkanke` だと名分ペナルティ | 不可 |
| `tachi` 館・城郭 | 収容兵力の上限 | 防衛拠点。攻城戦の舞台 | 不可 |
| `minato` 湊 | 船数 | 渡海と海戦の起点 | 水軍勢力の去就で移る |
