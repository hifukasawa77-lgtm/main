...
- **やりとりの自動記録**: Stop/SessionEnd hook（`.claude/hooks/second-brain-session-log.sh`）がセッションの会話ログを `obsidian-vault/01-Daily/sessions/` へ自動生成する（無効化: `SECOND_BRAIN_SESSION_LOG=0`）。CLAUDE.md全文は同期時に `04-Knowledge/claude-md-mirror.md` へ自動ミラーされる（ミラーは編集禁止）
- **スマホObsidian連携**: GitHub Actions `vault-sync.yml`（実体 `.github/scripts/vault-sync.sh`）が `obsidian-vault/` を専用Vaultリポジトリ（既定 `hifukasawa77-lgtm/obsidian-vault`・プライベート推奨）へ双方向同期し、スマホはObsidian Gitプラグインでそれをクローンする。セットアップ手順: `obsidian-vault/MOBILE-SETUP.md`
