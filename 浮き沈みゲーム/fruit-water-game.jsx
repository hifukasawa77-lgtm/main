...
const LEVELS = [
  ...
  { lv:5, name:'小さなカップ',  capacity:115, water:52, goal:16, color:'#9040c0', bg:'#e0c0ff', bowlW:210, innerW:176, preItems:['potato','onion','carrot'], preDesc:'じゃがいも・たまねぎ・にんじんが沈んでいる', icon:'🧪' },
];
...
1つ選び必ずJSONのみで返答:{"itemId":"ID","comment":"ひとこと30文字以内"}`;
  try{
    const res=await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({model:"claude-sonnet-4-6",max_tokens:150,system:"ゲームAI。JSON形式のみで返答。",messages:[{role:"user",content:prompt}]})});
      ...
      <div style={{
        ...
        {floaterLayout.map(({item,left,top},i)=>(
          <div key={item.instanceId} style={{
            ...
            animation:item.entering?'floatIn 0.5s cubic-bezier(0.34,1.56,0.64,1) forwards':`bob ${1.9+(i%5)*0.35}s ease-in-out infinite`,
...
function App(){
  ...
  function placeItem(itemDef, who){
    ...
    // Level clear check (solo) — checked before overflow so the goal-completing item wins
  ...
  return(
    <div style={{
      ...
      <style>{`
        ...
        @keyframes floatIn{from{opacity:0;transform:translateX(-50%) translateY(-20px) scale(0.7)}to{opacity:1;transform:translateX(-50%) translateY(0) scale(1)}}
                  ...
                  {ITEMS.map(item=>{
                    ...
                    return(
                      ...
                      >
                        ...
                        <span style={{fontSize:'0.58rem',color:disabled?'#9ca3af':'#374151',fontWeight:800,whiteSpace:'nowrap'}}>{item.name}</span>
