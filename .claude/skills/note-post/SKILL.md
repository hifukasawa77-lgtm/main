...
3. **失敗を書く**。「3コミット費やして誤診した」「自分の環境だけ動いていた」が記事の中心。
   成功談だけの記事は無料で足りてしまう
4. **一人称の経緯を作らない（最重要）**。「最初はこう作って失敗した」「2週間気づかなかった」
   「指摘が毎回20件出た」のような**経緯・回数・期間は、Vault か CLAUDE.md に該当記述がある
   ものだけ書く**。無ければ経緯として書かず、「この形は破綻する」という一般論の書き方にする。
   実装の説明も同じで、**コードを見て確かめた事実だけ**書く。
   **これは機械検査で捕まらない**（`verify-note-articles.mjs` はパスの実在と数字の総数しか見ない）。
   書く側が守るしかない唯一のルールなので、最優先で守ること。

   > **実際にやった失敗（2026-09-24, 深澤の指摘で発覚）**: 初回の3本で、
   > 「最初は品質ゲートを直列にして動かなかった」「hookのずれに2週間気づかなかった」
   > 「将棋AIは終盤だけ読みを深くする可変深さ」「`setTimeout(fn,0)` で探索を分割している」を
   > **記録にも実装にも無いまま書いた**。`shogi.html` は `depth=1` 固定で駒割り評価すら無く、
   > 探索を分割している箇所は1つも無かった。主張を通すために事実を作る動きが出るので、
   > **書き終えたら一人称の断定を1つずつ grep で裏取りする**こと
...
（数字・パス・コード片・経緯）。記憶や推測で書かない。

**裏取りの具体手順**: 実装について書くなら、その識別子を `grep` で実在確認する
（例: 「可変深さ」と書くなら `grep -nE 'depth' <file>` で本当に可変か見る）。
経緯・回数・期間について書くなら、`grep -r '<キーワード>' obsidian-vault/ CLAUDE.md` で
該当記述を見つける。**見つからなければ書かない**（「たぶんそうだろう」で書くと後で全部剥がす）。
