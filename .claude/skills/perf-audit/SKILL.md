...
**計測の限界（Canvasゲームでは過小評価される）**: 集計は HTML 内の `src`/`href` 属性の静的走査なので、
**JS内の文字列から動的に読み込むアセット（`new Image(); img.src = 'assets/…'` 等）は数えない**。
Canvasゲームは大半のアセットをこの経路で読むため、レポート値は実際のロード量を大きく下回る
（例: sengoku.html はレポート約4MBだが、シナリオ導入画像だけで別途約15.6MBを読む）。
ゲームHTMLを評価するときは、レポート値に加えて `assets/<ゲーム名>/` 配下の実サイズ
（`du -sh assets/<ゲーム名>`）を必ず併せて見ること。

**「0ページ計測」「対象0件」は結果ではなく故障**: そう出たら探索条件かスクリプト自体を疑う
（2026-07-28まで stdin 奪取のバグで常に0ページ計測＝偽の✅を返していた。詳細: [[harness-maintenance-patterns]]）。
